...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_TEST) || \
         ...
         
{
    ...
    do
    {
        UART_TEST_PutChar(pData[buf_index]);
        buf_index++;
         ...
         
{
    ...
    *  If atleast one byte is received within the timeout interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != UART_TEST_GetRxBufferSize())
        {
            ...
            */
            do
            {
                ...
                oldDataCount = UART_TEST_GetRxBufferSize();
                CyDelay(UART_TEST_BYTE2BYTE_TIME_OUT);
            }while(UART_TEST_GetRxBufferSize() > oldDataCount);
    ...
    *count = 0u;
    ...
    while(UART_TEST_GetRxBufferSize() > 0u)
    {
        tempCount = UART_TEST_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_TEST_ReadRxData();
                dataIndexCntr++;
            }
